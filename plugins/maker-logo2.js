let handler = async (m, { conn, args, text, usedPrefix, command }) => {
	let tee = `✳️ Enter a short text\n\n📌 Example  : *${usedPrefix + command}* Qin`
	let too = `✳️ Separate the text with a *+* \n\n📌 Example : \n*${usedPrefix + command}* Qin *+* BOT`
	let three= `✳️ Need three text seperated by *+* \n\n📌 Example : \n*${usedPrefix + command}* Qin *+* BOT *+* GOAT`
	...
	case 'gfx13': 
	if (!text) throw tee;
	let randirona = `https://api.lolhuman.xyz/api/ephoto1/mlwall?apikey=${lolkeysapi}&text=${encodeURIComponent(text)}`;
	conn.sendFile(m.chat, randirona, 'logo.png', `✅ Result`, m);
	m.react(done);
	break;

	case 'gfx14': 
	if (!text) throw three;
	if (!text.includes('+')) throw three  
	let [x,y,z] = text.split`+`;
	let randiron = `https://api.lolhuman.xyz/api/ephoto3/valorantbanner?apikey=${lolkeysapi}&text1=${encodeURIComponent(x.trim())}&text2=${encodeURIComponent(y.trim())}&text3=${encodeURIComponent(z.trim())}`;
	conn.sendFile(m.chat, randiron, 'logo.png', `✅ Result`, m);
	m.react(done);
	break;
	  default:
	} 
  ...
  handler.help = ['gfx1', 'gfx2', 'gfx3', 'gfx4', 'gfx5', 'gfx6', 'gfx7', 'gfx8', 'gfx9', 'gfx10', 'gfx11', 'gfx12','gfx13'];
  handler.tags = ['maker'];
  handler.command = /^(gfx1|gfx2|gfx3|gfx4|gfx5|gfx6|gfx7|gfx8|gfx9|gfx10|gfx11|gfx12|gfx13|gfx14)$/i;
