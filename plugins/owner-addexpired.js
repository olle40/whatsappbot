let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    // Ensure that global.db.data.chats[who].expired exists and is an object
    if (!global.db.data.chats[who]) global.db.data.chats[who] = {}
    if (!global.db.data.chats[who].expired) global.db.data.chats[who].expired = now
...
function msToDate(ms) {
    ...
    return [d, ' Days,', h, ' Hours\n ', m, ' Minutes ', s, ' Seconds '].map(v => v.toString().padStart(2, 0)).join('')
}
