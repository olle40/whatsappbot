...
const handler = async (m, { conn, args }) => {
  ...
  if (feature === 'start') {
    ...
    if (players.length === 2) {
      ...
      const boardUrl = `https://www.chess.com/dynboard?fen=${encodedFen}&size=3&coordinates=inside${flipParam}`;
      try {
...
handler.help = ['chess [from to]', 'chess delete', 'chess join', 'chess start','chess create'];
handler.tags = ['game'];
